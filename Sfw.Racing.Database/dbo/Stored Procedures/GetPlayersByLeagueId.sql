...
SELECT p.*, RANK() OVER (ORDER BY BasePoints + DriverPoints + EnginePoints + ConstructorPoints + QuestionPoints DESC) as [Position], BasePoints, DriverPoints, EnginePoints, ConstructorPoints, QuestionPoints FROM Player p 
...
ORDER BY BasePoints + DriverPoints + EnginePoints + ConstructorPoints + QuestionPoints desc